...
async def init_redis(redis_url:str)->Redis:
    ...
    try:
        ...
        await logger_admin.log_info("Redis connection established successfully.")
        return redis_client
    except Exception as e:
        await logger_admin.log_critical(f"Failed to connect to Redis: {e}")
        raise
...
async def get_redis_client(redis_url: str = root_config.REDIS_URL) -> Redis:
    ...
    if redis_client is None:
        await logger_admin.log_warning("Redis client is not initialized. Re-initializing.")
