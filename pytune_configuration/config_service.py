...
from pytune_logger.logger import logger_admin
...
class Config:
    ...
    async def initialize(self):
        ...
        """
        await self.logger.log_info(f"Starting PyTune Configuration Provider for table: {self.table_name}")
    ...
    async def load_configurations(self):
        ...
        """
        await self.logger.log_info(f"Loading configurations from table: {self.table_name}")
        async with config_lock:
            try:
                ...
                logger_admin.sync_log_info(f"RABBIT_BACKEND défini sur : {config_global.RABBIT_BACKEND}")
            
            except Exception as e:
                await self.logger.log_critical(f"Error while loading configurations from table {self.table_name}: {e}")
        ...
        async with config_lock:
            async with PostgresService.get_config_manager_pool().acquire() as conn:
                try:
                    ...
                    await logger_admin.log_critical(f"Error while updating config : {e}")
        ...
        async with config_lock:
            async with PostgresService.get_config_manager_pool().acquire() as conn:
                try:
                    ...
                    await logger_admin.log_critical(f"Error while deleting an iten from configuration : {e}")
    ...
    async def add_config_to_db(self, config_key: str, config_value: str, description: str = ""):
        ...
        async with config_lock:
            async with PostgresService.get_config_manager_pool().acquire() as conn:
                try:
                    await logger_admin.log_debug(f"Attempting to insert config_key={config_key}, config_value={config_value}, description={description}")
                    ...
                    await logger_admin.log_info(f"Insert result: {result}")
                    return {"message": "Configuration added successfully."}

                except asyncpg.UniqueViolationError:
                    await logger_admin.log_error(f"Configuration key '{config_key}' already exists.")
                    raise HTTPException(status_code=400, detail=f"Configuration '{config_key}' already exists.")  # Client response

                except asyncpg.DataError as e:
                    await logger_admin.log_error(f"Data type error while adding configuration: {e}")
                    raise HTTPException(status_code=400, detail=f"Data error: {e}")

                except asyncpg.Error as e:
                    await logger_admin.log_error(f"General database error while adding configuration: {e}")
                    raise HTTPException(status_code=500, detail="A database error occurred.")

                except Exception as e:
                    await logger_admin.log_error(f"Unexpected error while adding configuration: {e}")
    ...
    async def handle_config_change(self, message):
        ...
        try:
            if message.startswith("config_update:"):
                ...
                await self.logger.log_info(f"Configuration updated: {key} = {value}")
            elif message.startswith("config_delete:"):
                _, key = message.split("config_delete:")
                if hasattr(self, key):
                    delattr(self, key)
                    await self.logger.log_info(f"Configuration deleted: {key}")
                else:
                    await self.logger.log_warning(f"Attempted to delete non-existent key: {key}")
            elif message.startswith("config_add:"):
                _, config_data = message.split("config_add:")
                key, value = config_data.split(":", 1)
                setattr(self, key, value)
                await self.logger.log_info(f"Configuration added: {key} = {value}")
            elif message == "force_reload":
                await self.logger.log_info("Force reload of configurations triggered.")
                await self.load_configurations()
            else:
                await self.logger.log_warning(f"Unrecognized message received on 'config_change': {message}")
        except Exception as e:
            await self.logger.log_critical(f"Error handling config change message: {e}")
    ...
    async def reload_configurations(self):
        ...
        """
        await self.load_configurations()
        await self.logger.log_info("Configurations reloaded successfully.")
...
async def initialize_config(table_name: str = "configurations"):
    ...
    if config_global is None:
        logger_admin.sync_log_info(f"Initializing global Config for table '{table_name}'")
